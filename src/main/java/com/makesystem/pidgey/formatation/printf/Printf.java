 ...
 * result, depending on whether uppercase characters are being forced. For octal
 * conversions, the string "0" is prefixed to the result unless the initial
 * chracter of the result is already a "0"
 * .<P>
 * '0': Zero padding<br>
 * Zeros, rather than spaces, should be used to pad the formatted string. See
 * "-"
 * .<P>
 * '-': Left justification<br>
 * The formatted string is left justified. If both "-" and "0" are specified,
 * "-" has precedence.<P>
 * '+': Force sign<br>
 * For signed conversions, a "+" is prefixed to positive values. If both "+" and
 * " " are specified, "+" has precedence.<P>
 ...
 * The value being formatted will be padded out to the natural width of its
 * type. This is related to, but separate from, the width specification.<P>
 *
 * <u><b>Width</b></u>
 * This specifies the minimum length of the formatted string. If the formatted
 * string is naturally longer than this value, this value has no effect. However
 * if the formatted string is shorted than this value, it is padded with zero or
 * spaces as determined by the flags and conversion specifier.
 * <P>
 * <u><b>Precision</b></u>
 ...
 * %: This specifier does not consume an argument and prints "%"
 * .<P>
 * b: Takes a type supported for numeric conversions (see below) and prints the
 * value as binary. X/Z values are printed in lowercase by default. c: Takes any
 * type and converts it to a string. Then the first chracter is printed. Width
 * is respected, but only one character will be printed, so sprintf("%3c",
 * "foo") will print " f"
 * .<P>
 * d, i: Takes a type supported for numeric conversions (see below) and prints
 * the value as a signed, decimal value. If the argument contains X/Z values, a
 * "?" is printed.<P>
 ...
 * numeric values, that nibble is printed as "?". If such a nibble contains only
 * X values or only Z values, an "X" or "Z" is printed as appropriate. If the
 * top nibble of the value is not a full 3 bits, it is not zero-padded. For
 * example ("%o", 2'bxx) prints "X", not "?" (which would be the case if it was
 * zero-padded to 3'b0xx)
 * .<P>
 ...
 * used, any letters printed (a-f) will be printed as uppercase characters. If
 * the top nibble of the value is not a full 4 bits, it is not zero-padded (see
 * %o description for an example of how this affects formatting)
 * .<P>
 *
 * <u><b>Numeric Conversions</b></u><br>
 * Types supported for numeric conversions include:<br>
 * Boolean: <code>true</code> is converted to
 * {@link com.newisys.verilog.util.Bit#ONE} and <code>false</code> is converted
 * to {@link com.newisys.verilog.util.Bit#ZERO}
 * .<P>
 ...
 * character in the sequence and so on with the least most significant 8 bits of
 * the BitVector holding the ASCII value of the last character in the
 * sequence.<P>
 ...
 * bits. The most significant 8 bits are then interpreted as an ASCII value and
 * that character is printed, and so on. Should one of the bytes have a value of
 * zero, that byte is ignored, and the next byte is processed. This implies that
 * printing the value 0 as a string results in the empty string. Further, if the
 * argument contains X/Z values, the empty string is printed.<P>
 * All other types: All other types are printed using their
 * {@link Object#toString} methods.
 * <P>
 * <u><b>Notes for derived classes</b></u><br>
 * Derived classes must support format specifications with the following format:
 * <code>%[flags][width][.[precicion]]&lt;conversionSpecifier&gt;</code>, where
...
public class Printf {
    ...
    // base formatters
    private final ConversionFormatter sFormatter = new StringFormatter();
    private final ConversionFormatter nFormatter = new NumericFormatter();
    private final ConversionFormatter noArgFormatter = new NoArgFormatter();
    ...
    private final ConversionFormatter[] formatters = new ConversionFormatter[128];
    ...
    public char getPaddingChar() {
        return sFormatter.getPaddingChar();
    }

    public void setPaddingChar(final char paddingChar) {
        sFormatter.setPaddingChar(paddingChar);
        nFormatter.setPaddingChar(paddingChar);
        noArgFormatter.setPaddingChar(paddingChar);
    } 
     ...
     */
    public final synchronized String toString(final CharSequence fmt, final Object... args) {
        int curArgIdx = 0;
        int curSpecIdx = 0;
        ...
        for (int i = 0; i < length; ++i) {
            ...
            if (processingPercent) {
                if (processingFlags) {
                    ...
                    final PrintfFlag flag = getFlag(c);
     ...
     * performed
     */
    private CharSequence postProcess(final CharSequence buf) {
        return buf;
    }
     ...
     *
     * @param conversionSpec the conversion specifier for which to return a
     * ConversionFormatter
     * @return a ConversionFormatter for <code>conversionSpec</code>
     */
    private ConversionFormatter getFormatter(final char conversionSpec) {
        return formatters[conversionSpec];
    }

    /**
     * Returns the {@link PrintfFlag} associated with the specified flag or
     * <code>null</code> if <code>flag</code> does not correspond to a supported
     * printf flag.
     *
     ...
     */
    private PrintfFlag getFlag(final char flag) {
        switch (flag) {
            case '#':
     ...
     */
    private boolean isNumericConversion(final char conversionSpec) {
     ...
     * conversion, <code>false</code> otherwise
     */
    private boolean isStringConversion(final char conversionSpec) {
        switch (conversionSpec) {
            case 'c':
            case 's':
                return true;
     ...
     * conversion, <code>false</code> otherwise
     */
    private boolean isValidConversion(final char conversionSpec) {
     ...
     * specifier, or <code>null</code> if no ConversionFormatter exists for the
     * specifier.
     *
     * @param conversionSpec the conversion specifier to check
     ...
     */
    private ConversionFormatter getFormatterForSpec(final char conversionSpec) {
        if (isNumericConversion(conversionSpec)) {
            return nFormatter;
     ...
     * value is exactly the number of conversion specifiers in the format that
     * consume an argument.
     */
    private void warnAboutMissingArgs(final int observedArgs, final int expectedArgs) {
        ...
        for (int i = 0; i < stack.length; ++i) {
            final StackTraceElement level = stack[i];
            if (level.getClassName().equals(Printf.class.getName())) {
