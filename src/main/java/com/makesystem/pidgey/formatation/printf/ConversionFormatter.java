...
public interface ConversionFormatter {
    ...
    /**
     * Returns the maximum length of the string produced by the given PrintfSpec
     * when formatted. As its name suggests, this need not be exact, but it
     * should err on the side of returning a value larger than what will
     * actually be needed.
     * <P>
    ...
    /**
     * Formats the given PrintfSpec, appending it to the specified
     * StringBuilder.
     * <P>
     * Note that <code>spec</code> will be the exact PrintfSpec passed to
